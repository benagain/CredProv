...
namespace Matchers {
    ...
    namespace StdString {
		...
		template<typename T>
        struct Contains : MatcherImpl<Contains<T>, T> {
            Contains( T const& substr ) : m_substr( substr ){}
            Contains( Contains const& other ) : m_substr( other.m_substr ){}

			virtual ~Contains() { }

            virtual bool match( T const& expr ) const {
                return expr.find( m_substr ) != T::npos;
            }
            virtual std::string toString() const {
				return "contains: \"" + Catch::toString(m_substr) + "\"";
            }
		...
		template<typename T>
        struct StartsWith : MatcherImpl<StartsWith<T>, T> {
            StartsWith( T const& substr ) : m_substr( substr ){}
		...
		template<typename T>
        struct EndsWith : MatcherImpl<EndsWith<T>, T> {
            EndsWith( T const& substr ) : m_substr( substr ){}
	...
	template<typename T>
	inline Impl::StdString::Contains<std::basic_string<T>>    Contains(std::basic_string<T> const& substr) {
		return Impl::StdString::Contains<std::basic_string<T>>(substr);
    }
	template<typename T>
	inline Impl::StdString::Contains<std::basic_string<T>>    Contains(const T* substr) {
		return Impl::StdString::Contains<std::basic_string<T>>(substr);
    }
	template<typename T>
    inline Impl::StdString::StartsWith<std::basic_string<T>>  StartsWith(std::basic_string<T> const& substr ) {
        return Impl::StdString::StartsWith<std::basic_string<T>>( substr );
    }
	template<typename T>
    inline Impl::StdString::StartsWith<std::basic_string<T>>  StartsWith( const T* substr ) {
        return Impl::StdString::StartsWith<std::basic_string<T>>( substr );
    }
	template<typename T>
    inline Impl::StdString::EndsWith<std::basic_string<T>>    EndsWith(std::basic_string<T> const& substr ) {
        return Impl::StdString::EndsWith<std::basic_string<T>>( substr );
    }
	template<typename T>
	inline Impl::StdString::EndsWith<std::basic_string<T>>    EndsWith( const T* substr ) {
        return Impl::StdString::EndsWith<std::basic_string<T>>( substr );
    }
