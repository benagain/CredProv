...
IFACEMETHODIMP MyidCredential::Advise(__RPC__in_opt ICredentialProviderCredentialEvents* pcpce)
{
...
IFACEMETHODIMP MyidCredential::UnAdvise()
{
	if (m_events)
		m_events.Release();
	if (m_events2)
...
IFACEMETHODIMP MyidCredential::SetSelected(__RPC__out BOOL* pbAutoLogon)
{
	try
	{
		*pbAutoLogon = FALSE;
...
IFACEMETHODIMP MyidCredential::SetDeselected()
{
	try
	{
		m_strings[Fields::FID_NAME] = m_data.name;
...
IFACEMETHODIMP MyidCredential::GetFieldState(__RPC__in DWORD fieldID, __RPC__out CREDENTIAL_PROVIDER_FIELD_STATE* pcpfs, __RPC__out CREDENTIAL_PROVIDER_FIELD_INTERACTIVE_STATE* pcpfis)
{
	try
	{
...
IFACEMETHODIMP MyidCredential::GetStringValue(__RPC__in DWORD fieldID, __RPC__deref_out_opt_string LPWSTR* ppsz)
{
	*ppsz = nullptr;
...
IFACEMETHODIMP MyidCredential::GetBitmapValue(__RPC__in DWORD fieldID, __RPC__deref_out_opt HBITMAP* phbmp)
{
	*phbmp = nullptr;
...
IFACEMETHODIMP MyidCredential::GetCheckboxValue(__RPC__in DWORD fieldID, __RPC__out BOOL* pbChecked, __RPC__deref_out_opt_string LPWSTR* ppszLabel)
{
...
IFACEMETHODIMP MyidCredential::GetSubmitButtonValue(__RPC__in DWORD fieldID, __RPC__out DWORD* pdwAdjacentTo)
{
...
IFACEMETHODIMP MyidCredential::GetComboBoxValueCount(__RPC__in DWORD fieldID, __RPC__out DWORD* pcItems, _Deref_out_range_(<, *pcItems) __RPC__out DWORD* pdwSelectedItem)
{
	UNREFERENCED_PARAMETER(fieldID);
...
IFACEMETHODIMP MyidCredential::GetComboBoxValueAt(__RPC__in DWORD fieldID, __RPC__in DWORD dwItem, __RPC__deref_out_opt_string LPWSTR* ppszItem)
{
	UNREFERENCED_PARAMETER(fieldID);
...
IFACEMETHODIMP MyidCredential::SetStringValue(__RPC__in DWORD fieldID, __RPC__in_string LPCWSTR psz)
{
	try
	{
		switch (fieldID)
...
IFACEMETHODIMP MyidCredential::SetCheckboxValue(__RPC__in DWORD fieldID, __RPC__in BOOL bChecked)
{
	UNREFERENCED_PARAMETER(fieldID);
...
IFACEMETHODIMP MyidCredential::SetComboBoxSelectedValue(__RPC__in DWORD fieldID, __RPC__in DWORD dwSelectedItem)
{
	UNREFERENCED_PARAMETER(fieldID);
...
IFACEMETHODIMP MyidCredential::CommandLinkClicked(__RPC__in DWORD fieldID)
{
	UNREFERENCED_PARAMETER(fieldID);
...
IFACEMETHODIMP MyidCredential::GetSerialization(__RPC__out CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE* pcpgsr, __RPC__out CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION* pcpcs, __RPC__deref_out_opt_string LPWSTR* ppszOptionalStatusText, __RPC__out CREDENTIAL_PROVIDER_STATUS_ICON* pcpsiOptionalStatusIcon)
{
...
IFACEMETHODIMP MyidCredential::GetUserSid(__RPC__deref_out_opt_string LPWSTR* sid)
{
	*sid = nullptr;
...
IFACEMETHODIMP MyidCredential::GetFieldOptions(__RPC__in DWORD fieldID, __RPC__out CREDENTIAL_PROVIDER_CREDENTIAL_FIELD_OPTIONS* options)
{
	if (fieldID >= Fields::FID_NUM_FIELDS)
		return E_INVALIDARG;
